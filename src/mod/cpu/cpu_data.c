...
uint64_t get_temp_id()
{
   ...
   {
      if (entry->d_name[0] == '.' ||
          strncmp(entry->d_name, HWMON, sizeof(HWMON) - 1) != 0)
         continue;
...
void get_topology(CpuMon cpumon[static 1])
{
   ...
   cpumon->phy_count =
      (uint16_t)cpumon->thread_count / cpumon->threads_per_core;
}
...
static inline uint8_t calc_core_usage(uint64_t user,
                                      uint64_t nice,
                                      uint64_t system,
                                      uint64_t idle,
                                      uint64_t iowait,
...
static inline void push_to_history(CpuMon cpumon[static 1],
                                   int cpu_id,
                                   uint8_t usage)
{
...
void parse_stats(CpuMon cpumon[static 1])
{
   ssize_t bytes_read = pread(
      cpumon->fd_stat, cpumon->stat_buffer, cpumon->stat_buffer_size - 1, 0);
   if (bytes_read < 0)
      return;
   ...
   if (*p == 'c')
   {
      skip_to_digit(&p);
      skip_to_digit(&p);
      uint64_t user = str_to_uint64(&p);
      skip_to_digit(&p);
      uint64_t nice = str_to_uint64(&p);
      skip_to_digit(&p);
      uint64_t system = str_to_uint64(&p);
      skip_to_digit(&p);
      uint64_t idle = str_to_uint64(&p);
      skip_to_digit(&p);
      uint64_t iowait = str_to_uint64(&p);
      skip_to_digit(&p);
      uint64_t irq = str_to_uint64(&p);
      skip_to_digit(&p);
      uint64_t softirq = str_to_uint64(&p);
      skip_to_digit(&p);
      uint64_t steal = str_to_uint64(&p);

      skip_line(&p);

      uint8_t global_usage = calc_core_usage(user,
                                             nice,
                                             system,
                                             idle,
                                             iowait,
   ...
   for (size_t cpu_id = 0; cpu_id < cpumon->thread_count; cpu_id++)
   {
      ...
      uint8_t usage = calc_core_usage(user,
                                      nice,
                                      system,
                                      idle,
                                      iowait,
