...
#define APPEND_LIT(buf, str) \
   ((void)(memcpy(*buf, str, sizeof(str) - 1), *buf += sizeof(str) - 1))
...
static inline uint64_t str_to_uint64(char** buf)
{
   uint64_t val = 0;
   while (**buf >= '0' && **buf <= '9')
   {
...
static inline void skip_to_digit(char** buf)
{
   while (**buf && (**buf < '0' || **buf > '9'))
      (*buf)++;
}

static inline void skip_line(char** buf)
{
   while (**buf && (**buf != '\n'))
      (*buf)++;
...
#define append_num(buf, val)               \
   _Generic((val),                         \
      ...
      long long: append_i64_base)(buf, val)

static inline void append_two_digits(char** buf, int val)
{
   if (val < 10)
   {
      *(*buf)++ = '0';
...
static inline void append_fixed_shift_1d(char** buf,
                                         uint64_t val,
                                         uint64_t shift)
{
...
static inline void append_fixed_shift_2d(char** buf,
                                         uint64_t val,
                                         uint64_t shift)
{
   append_u64_base(buf, val >> shift);
...
static inline void append_fixed_generic(char** buf,
                                        uint64_t val,
                                        uint64_t divisor,
                                        uint64_t prec_mult)
{
...
static inline void append_fixed_shift_generic(char** buf,
                                              uint64_t val,
                                              uint64_t shift,
                                              uint64_t prec_mult)
{
